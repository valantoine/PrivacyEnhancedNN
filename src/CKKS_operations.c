...
// void random_power_of_2_below(mpz_t g, gmp_randstate_t state, mpz_t A)
// {
   
//     int max_exp = 0;
//     while ((mpz_t)1 << (max_exp + 1) < A)
//         max_exp++;

//     int exp = max_exp / 2 + rand() % (max_exp / 2 + 1); // return an random exp between max_exp / 2 and max_exp
//     return (mpz_t)1 << exp;                           // 2^exp;
// }

// // Should be pre computed at the beginning with every modulo levels : encryption of g*(S^2) mod gq.
// ciphered_t *evk_g(polynomial_t *secret_key, mpz_t current_modulo, mpz_t g)
// {
...
// ciphered_t *ckks_mult_cipher_cipher(ciphered_t *c1, ciphered_t *c2, mpz_t modulo,  ciphered_t *evk_g_precomputed, mpz_t g)
// {
