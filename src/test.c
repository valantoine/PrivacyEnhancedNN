...
void test_encrypt_decrypt()
{
    ...
    mpz_init_set_ui(encoding_precision_factor, 512);
    mpz_t scaling_factor;
    mpz_init_set_ui(scaling_factor, 64);
    complex_vector_t *complex_vector = complex_vector_init(size);
...
//     size_t size = 2;
//     int64_t encoding_precision_factor = 64;
//     int64_t scaling_factor = 2048;
//     complex_vector_t *complex_vector1 = complex_vector_init(size);
...
//     fprintf(stdout, " Key : \n");
//     polynomial_print(secret_key);
//     int64_t modulo = ((int64_t)1 << 32) * scaling_factor; // i.e q_0 = 2^16, L=1, delta= scaling_factor
//     ciphered_t *c1 = encrypt(encoded_pol1, secret_key, modulo, scaling_factor);
//     ciphered_t *c2 = encrypt(encoded_pol2, secret_key, modulo, scaling_factor);
//     fprintf(stdout, "First ciphered vector : \n");
//     cipher_print(c1);
...
//     mpz_t g;
//     mpz_init(g);
//     random_power_of_2_below(modulo); // normalement modulo * modulo mais trop grand ici
//     fprintf(stdout, "g = %" PRId64 "\n", g * modulo);
//     ciphered_t *evk_g_precompute = evk_g(secret_key, modulo, g);
//     ciphered_t *mult = ckks_mult_cipher_cipher(c1, c2, modulo, evk_g_precompute, g);
